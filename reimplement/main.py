...
def surface_context(sentence, aspect_term,from_char,to_char, window_size):  # from_index is the begin of the aspect term in list sentence.strip()
        ...
        for aspectTerms in sentence.iter('aspectTerms'):
            for aspectTerm in aspectTerms.iter('aspectTerm'):
                text.append(sentence_transform(content, aspectTerm.get('term').translate(string.maketrans("", ""),
                                                                                         string.punctuation),aspectTerm.get("from"),aspectTerm.get("to"), 8))
